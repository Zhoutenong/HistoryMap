...
> - [x] 州府级数据管线（元丰九域志 1080 基准：州府近似边界 + 治所 + 户口/土贡/沿革，`docs/architecture/data-improvement-plan.md`）
> - [x] 宋代时空数据库（PostgreSQL + PostGIS 时间版本化，`docs/architecture/temporal-db-plan.md`）
> - [x] Android 原生重构落地（Kotlin + Compose + GLES2 自研渲染器，弃用 WebView 壳；`docs/architecture/android-native-rewrite-plan.md`）
> - [x] 水彩疆域贴图烘焙管线（8 张 penpot-v1 贴图，Web/Android 双端接入；`docs/technical/texture-bake-plan.md`）
> - [x] M1/M2 视觉调优（字体/水彩/色相/河道，P20 实测 55–59fps）
>
> 下述早期 Web 端条目为 Web 功能路线图；Android 原生版实现与验收见 `docs/requirements/refactor-requirements.md`。
